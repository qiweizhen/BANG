...
@register_task('ngram_masked_s2s')
class NgramMaskedS2STask(FairseqTask):
    """
    ...
    def add_args(parser):
        ...
        parser.add_argument('--distort-s2s-prob', default=0.0, type=float,
                            help='probability of replacing a token with mask')
    ...
    def build_s2s_dataset(self, dataset):
        ...
        s2s_dataset = MaskedLanguagePairDataset(
            ...
            seed=self.args.seed,
            distort_prob=self.args.distort_s2s_prob,
        )
        return s2s_dataset

    def build_dataset_for_inference(self, src_tokens, src_lengths):
        # print("src_tokens", src_tokens)
        dataset = PadDataset(TokenBlockDataset(
            src_tokens,
            src_lengths,
            self.args.tokens_per_sample,
            pad=self.source_dictionary.pad(),
        ...
        s2s_dataset = MaskedLanguagePairDataset(
            dataset, dataset.sizes, self.source_dictionary,
            shuffle=False, mask_prob=self.args.mask_s2s_prob,
            pred_probs=pred_probs,
            seed=self.args.seed,
            distort_prob=self.args.distort_s2s_prob,
        )
