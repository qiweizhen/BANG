...
class MaskedLanguagePairDataset(LanguagePairDataset):
    ...
    def collate(self, samples, pad_idx, eos_idx, left_pad_source=True, left_pad_target=False):
        ...
        target_idx = merge([s['target_idx'] for s in samples], left_pad=left_pad_target)
    ...
    def distort(self, x):
        _x = x.clone()
        seq_len = x.size(-1)
        position = np.arange(seq_len - 1, step=2)
        np.random.shuffle(position)
        distort_position = position[:int(seq_len*self.distort_prob)]
        _x[distort_position] = x[distort_position+1]
        _x[distort_position+1] = x[distort_position]
        return _x
